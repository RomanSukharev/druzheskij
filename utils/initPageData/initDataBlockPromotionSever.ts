...
export default (
  path: string,
  data_block_promotion_sever: Ref<IBlockPromotionSever>,
  links_plan_group: Ref<INavGroup[]>,
  response_data: IResponse
) => {
  var promo = response_data?.payload.objects[0].promotion.find((i) => i.anchor_link === 'sever')
  ...
  switch (path) {
    case '/sever': {
      ...
      data_block_promotion_sever.value.house_list = house_list
      data_block_promotion_sever.value.min_payment = promo.description!.slice(3, -4)
